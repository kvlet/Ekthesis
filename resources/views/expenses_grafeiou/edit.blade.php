...
@section('formsdetails')
    <form id="create_eksoda_grafeiou_form" method="post" action="{{ route('graf_expen.update') }}" autocomplete="off">
        @csrf
        {{ method_field('PUT') }}
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <input name="id_grafeio" type="hidden" value="{{ $expen_graf->id_grafeio }}">
                                <input name="id_expenses" type="hidden" value="{{ $expen_graf->id_expenses }}">
                                <input name="Date_expen" type="hidden" value="{{ $expen_graf->Date_expen }}">
                                <div class="form-label{{ $errors->has('id_grafeio') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-select" id="id_grafeio" name="id_grafeio" required disabled >
                                        @foreach($grafeia as $grafeiο)
                                            <option value="{{$grafeiο->id_grafeio}}" @if($expen_graf->id_grafeio == $grafeiο->id_grafeio) selected @endif>{{ $grafeiο->Name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-label{{ $errors->has('id_expenses') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-select" id="id_expenses" name="id_expenses" required disabled>
                                        @foreach($expenses as $expen)
                                            <option value="{{$expen->id_expenses}}" @if($expen_graf->id_expenses == $expen->id_expenses) selected @endif>{{ $expen->eksodo }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-label{{ $errors->has('Date_expen') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="Date_expen" id="Date_expen" data-provide="datepicker" data-date-format="dd-mm-yyyy" placeholder="HH-MM-YYYY"
                                           ...
                                           value="{{ $expen_graf->Date_expen }}"  required disabled>
                            ...
                            <div class="col-md-4">
                                <div class="form-label{{ $errors->has('Value_expen') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="number" step="0.01" name="Value_expen" id="Value_expen"
                                           ...
                                           value="{{ $expen_graf->Value_expen }}" required>
                            ...
                            <div class="col-md-4">
                                <div class="form-label{{ $errors->has('Valid') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-select" id="Valid" name="Valid" required>
                                        <option value="true" @if($expen_graf->Valid=='true') selected="selected" @endif>Έγκυρο</option>
                                        <option value="false" @if($expen_graf->Valid=='false') selected="selected" @endif>Άκυρο</option>
                                    </select>
                                    @if ($errors->has('Valid'))
                                        <span class="invalid-feedback" role="alert">
